...
func NewServer(version string) *ServerState {
...
func (s *ServerState) runDiagnostic(doc *Document, notify glsp.NotifyFunc, delay bool) {
	...
	go func() {
		...
		for _, result := range result.Results {
			severity := getDiagnosticSeverityFromRule(result.Rule)
			diagnosticErrorHref := fmt.Sprintf("%s/rules/%s/%s,", model.WebsiteUrl, result.Rule.RuleCategory.Id, result.Rule.Id)
			diagnostics = append(diagnostics, protocol.Diagnostic{
				Range: protocol.Range{
					Start: protocol.Position{Line: protocol.UInteger(result.StartNode.Line - 1), Character: protocol.UInteger(result.StartNode.Column - 1)},
					End:   protocol.Position{Line: protocol.UInteger(result.EndNode.Line - 1), Character: protocol.UInteger(result.EndNode.Column + len(result.EndNode.Value) - 1)},
				},
				Severity:        &severity,
				Source:          &serverName,
				Code:            &protocol.IntegerOrString{Value: result.Rule.Id},
